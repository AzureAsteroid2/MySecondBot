...
class Users():
  ...
  async def gang_lookup(self, ctx, file):
    ...
    all_users = []
    ctx = int(ctx)
    with open(f'MySecondBot/Modules/Text_Files/{file}.txt', 'r') as temp:
      for line in temp:
  ...
  async def gang_add(self, ctx, userid, file):
    ...
    if self.present == False:
      with open(f'MySecondBot/Modules/Text_Files/{file}.txt', 'a') as temp:
        temp.write(f"`{userid}")
        temp.close()
    return
  ...
  async def gang_remove(self, ctx, userid, file):
    await self.gang_lookup(userid, file)
    if self.present:
      temp = open(f'MySecondBot/Modules/Text_Files/{file}.txt', 'r')
      all_users = temp.read()
      all_users = all_users.split('`')
      if userid in all_users:
        all_users.remove(userid)
      done = "`".join(all_users)
      temp = open(f'MySecondBot/Modules/Text_Files/{file}.txt', 'w')
      temp.write(done)
    return
